...
<c:set var="energycostList" value="${dataMap.energycostVOList }" />
<c:set var="energyMonthList" value="${dataMapMon.energyMonthList}" />
...
<br/><div id="print">
		<div class="col-12">
			<div class="callout callout">
				<h5 style="text-align: center;">월간 에너지 비용 분석</h5>
			</div>
		</div>
			<!-- 그래프 -->
		<div class="row">
			<div class="col-md-8">
				<div class="card-body">
					<div id="bar-chart" style="height: 500px; padding: 0px; position: relative;">
						<canvas id="barChart" style="min-height: 250px; height: 500px; max-height: 500px; max-width: 100%; display: block; width: 408px;" width="408" height="450" class="chartjs-render-monitor"></canvas>
					</div>
					<br/>
					<table class="table table-bordered">
						<thead>
							<tr>
								<th class="text-center">월 평균 에너지 비용</th>
								<th class="text-center">월 누적 에너지 비용</th>
							</tr>
						</thead>
						<tbody>
							<c:if test="${not empty dataMap.energycostList}">
								<c:set var="totalCost" value="0" />
								<c:forEach items="${dataMap.energycostList}" var="energycost" >
									<c:set var="totalCost" value="${totalCost + energycost.energyCost}" />
								</c:forEach>
								<c:set var="averageCost" value="${totalCost / fn:length(dataMap.energycostList)}" />
							</c:if>
						<tr class="odd">
				...
				<table id="cost" class="table table-bordered dataTable dtr-inline" aria-describedby="example1_info">
						...
						<c:forEach items="${dataMap.energycostList }" var="energycost" varStatus="loop">
							<tr class="odd">
								...
								<c:if test="${dataMap.energycostList.size() != loop.count}">
									...
									<c:set var="deviation" value="${(preCost - energycost.energyCost)}" />
						        	<c:set var="growRate" value="${deviation/preCost*100 }" />
									<td class="text-center"><fmt:formatNumber value="${deviation }" pattern="#,###"/></td>
									<td class="text-center"><fmt:formatNumber type="number" value="${growRate}" maxFractionDigits="1"/>%</td>
						        </c:if>
								<c:if test="${dataMap.energycostList.size() == loop.count}">
					...
					<div class="card-footer">
						<div style="display:${!empty dataMap.energycostList ? 'visible':'none' };">
							<%@ include file="/WEB-INF/views/energycost/month_analysispagination.jsp" %>
						</div>
					</div>
				</div>
			</div>
